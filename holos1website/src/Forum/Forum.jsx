...
export default function Forum() {
  ...
  const loadPosts = useCallback(() => {
    ...
    for (let i = 1; i <= 4; i++) {
      fetch(
        `https://wm0sx9jx6f.execute-api.us-east-1.amazonaws.com/dep/forum?page=${i}`
      )
        .then((res) => res.json())
        .then((data) => {
          if (i === currentPage) {
    ...
    <>
      ...
      <Container>
        ...
        <Form className="forum-content">
          <Form.Group>
            <Form.Label className="title-label" htmlFor="title">
              Title
            </Form.Label>
            <Form.Control
          ...
          <Form.Group>
            <Form.Label className="content-label" htmlFor="content">
              Content
            </Form.Label>
            <Form.Control
          ...
          <Button onClick={handlePost} className="submit-post-button">
            Post
          </Button>
        </Form>
        <hr />
                    ...
                    {post.isReplying && (
                      <Form.Group>
                        <Form.Control
                          ...
                          onChange={(e) => {
                            const newPosts = posts.map((p) =>
                              p.messageid === post.messageid
                                ? { ...p, replyContent: e.target.value }
                                : p
                            );
                            setPosts(newPosts);
                          }}
